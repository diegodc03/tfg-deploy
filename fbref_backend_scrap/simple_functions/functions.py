...
def fill_stats_dict(spark_df, expected_columns, dict_avg, dict_desv, dict_mode, table_name=None):
    count = spark_df.count()
    if count > 0:
        avg, max, mode = loop_throgh_all_columns_basic_positions(spark_df, count, table_name)
        dict_avg.update(avg)
