...
from function_type_of_competitions import get_possible_leagues
...
from spark_schema import get_season_schema, get_tournament_schema, possible_leagues_schema
...
def insert_league_competition_on_database(spark, url, db_properties):
    ...
    schema = possible_leagues_schema()

    # Crear DataFrame con las ligas
    leagues_df = spark.createDataFrame(data, schema)

    # Primero, insertar las temporadas
    all_seasons = []
    all_tournaments = []
    ...
    for league in data:
        ...
        for season in league['seasons']:
            all_tournaments.append({
                ...
                'tournament_fbref_id': league['id'],  # Usamos el mismo ID para el torneo
                'season_year': season
            })
...
def insert_seasons(seasons, spark, jdbc_url, db_properties):
    ...
    season_df = spark.createDataFrame(seasons, season_schema)
...
def insert_tournaments(tournaments, spark, jdbc_url, db_properties):
    ...
    tournament_df = spark.createDataFrame(tournaments, tournament_schema)

    tournament_df_to_insert = spark.createDataFrame([], tournament_schema)
    # Comprobar e insertar solo los torneos que no existen
