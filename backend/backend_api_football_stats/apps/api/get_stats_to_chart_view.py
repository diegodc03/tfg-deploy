...
from ..serializers_dto.serializers import GameModeSerializer, MatchPlayersScoreSerializer
...
# GET /api/statsToChart/?match_id=1&basic_position=1&type_table_stats=1

class GetStatsToChartView(APIView):
    ...
    def get(self, request):
            ...
            if not football_match:
                ...
                if not match_stats.exists():
                    ...
                    if type_table_stats not in stats_columns:
                        ...
                        modelo = model_map.get(type_table_stats)
                        if not modelo:
        ...
        for key, validator in param_validators.items():
            ...
            if not validator(value):
                errores[key] = f"El parámetro '{key}' no es válido."
        
        
        if 'match_id' in errores:
            if not match_id or not match_id.isdigit():
                errores['match_id'] = "El parámetro 'match_id' debe ser un número."
            response_data = {"error": errores['match_id']}
            response_status = status.HTTP_404_NOT_FOUND
        else:
            if 'type_table_stats' not in errores:
                if type_table_stats not in stats_columns + stats_columns_player:
                    errores['type_table_stats'] = "El parámetro 'type_table_stats' no es reconocido."
            ...
            for key, value in parametros.items():
                if value and key in ['basic_position_id', 'type_table_stats']:
                    match_stats = match_stats.filter(**{key: int(value)})

            if not match_stats.exists():
                response_status = status.HTTP_204_NO_CONTENT
                response_data = {"error": "No se encontraron estadísticas para este partido."}
            else:
                response_data = MatchPlayersScoreSerializer(match_stats, many=True)
                response_status = status.HTTP_200_OK
